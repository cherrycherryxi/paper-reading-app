...
前端不新增底部 Tab，不跳转或嵌入 dsh Web UI。用户从现有「探讨」自然开始，需要多步取证时再显式升级为「深入研究」；系统可以给出升级建议，但不会自动启动任务。书籍详情和摘抄详情保留带上下文的直达入口。
...
```json
{
  "query": "自由与责任",
  "relation_scope": "all",
  "exclude_current_book": true,
  "book_ids": ["book-xxx"],
  "limit": 20
}
```

第一版使用 SQLite 关键词检索，覆盖正文、OCR 文本、反思、标签、书名和作者。跨书问题首次检索排除当前书；关键词零命中时先用 `list_books` 确定目标书，再按 `book_ids` 宽召回目标书摘抄，不允许退化成全书架空查询。先验证多步检索是否有价值，再决定是否增加 embedding。
...
| 路径 | 用途 | 后端链路 |
| --- | --- | --- |
| 默认探讨 | 一问一答、解释、总结、提炼问题、整理笔记 | 现有 `/api/chat` |
| 深入研究 | 多轮检索、跨书比较、观点地图、长任务 | Research Run → Gateway → dsh |

dsh Web UI 只用于开发调试。正式用户始终停留在 paper-reading-app 内，不使用 iframe，不跳转到 `127.0.0.1:3080`。

### 8.2 主入口：从当前问题升级

输入跨书、对照、查证等研究型问题时，在输入区上方显示非阻塞建议；用户也可以随时点击输入框下方的「深入研究」。两种方式都只复制当前问题与书籍/摘抄上下文，不自动启动任务。

```html
<button id="chatResearchModeBtn" type="button">✦ 深入研究</button>
```

进入研究工作区后显示返回入口；研究完成后可把结论摘要带回普通输入框继续追问：
...
书籍详情操作区保留现有“去聊”，增加次级动作“深入研究”。点击后：
