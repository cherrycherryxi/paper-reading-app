...
4. 将 optimization/triage.md 的 Last triaged 更新为 ${TODAY}，并只指派一个最高优先级、未完成、可在单 PR 内完成的项；写明理由、关键文件、signal/Theme。
5. 只允许修改 optimization/triage.md、optimization/backlog.md 和 .wolf/。不要修改 roadmap/signals/explore 或应用代码。
6. 回复末尾输出单独成行的 <<<SUMMARY_START>>> 与 <<<SUMMARY_END>>>，中间写 200 字以内中文摘要。
