...
async function generateBookReview(bookId, textarea) {
  ...
  try {
    ...
    if (reply) {
      ...
      pendingAiReview = { bookId, text: reply };
...
// 保存读后感时判定来源（OPT-101）：本次「✨ AI 起草」出来的原文一字未改 → AI 来源；
// 用户改过一个字就当作自己写的；正文没动过则沿用书上已有的标记（重开对话框只改页码
// 后保存，不该把 AI 标记洗掉）。清空读后感自然回到无标记。
function isAiAuthoredReview(book, nextReview) {
  if (!nextReview) return false;
  if (pendingAiReview && pendingAiReview.bookId === book.id && pendingAiReview.text === nextReview) return true;
  return nextReview === (book.review || "").trim() && Boolean(book.reviewIsAi);
}
...
async function addBook(formData) {
  ...
  try {
    ...
    state.books.unshift({
      ...
      reviewIsAi: false, // 新增书的表单没有「AI 起草」按钮，这里的读后感一定是手写的
...
async function saveBookEdit(formData) {
  ...
  book.reviewIsAi = isAiAuthoredReview(book, nextReview);
  book.review = nextReview;
