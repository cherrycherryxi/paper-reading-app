...
- [2026-07-17] **非交互 session（daily loop / launchd 起的 agent）里 `git push`、`gh pr create`、`gh pr merge` 一定会被拒——`~/.claude/settings.json` 的 `permissions.ask` 列了这三条，ask 在无人应答时只能 fallback 成拒绝。** 项目 `.claude/settings.local.json` 里有 `Bash(git push *)` 的 allow 也没用：全局 ask 优先。表现是 `Claude requested permissions to use Bash, but you haven't granted it yet.`，不是网络/沙箱错误，`dangerouslyDisableSandbox: true` 也绕不过（沙箱与权限是两套闸）。**别反复重试或换命令写法浪费轮次**——认出这个报错就直接：代码+测试做完、各分支本地提交、按 loop 约定写 NEED-HUMAN 留给人工推。要让 loop 能自己开 PR，得由 owner 把这三条从 `ask` 移到 `allow`（注意这会同时放开 `git push` 到任何分支，与「绝不 push main」的护栏冲突，需先想清替代护栏，如 `.githooks/pre-push` 拦 main）。
