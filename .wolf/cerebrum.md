...
- [2026-07-19]（bug-545）**串联的定时 routine 之间只留 1 小时 cron 间隔，会被排队延迟颠倒执行顺序——上游延迟 > 间隔时，下游读到过期产物。** 云端 Agent1(triage, cron `0 17 * * *` UTC)与 Agent2(implement, `0 18 * * *`)名义隔 1h。07-18 夜 Agent1 排队延迟 68min(实际 18:08 UTC 落地)、Agent2 仅延迟 3min(18:03)→ Agent2 抢先跑 → 读到前一天的 triage.md → `FRESHNESS CHECK` 判过期 → 整晚 STOP 零产出。症状极隐蔽:没有报错、没有失败 PR，只是"自主轨安静地什么都不做"，owner 误判成"预算没生效"。**排查串联 routine 的"没产出"时，先比 `last_fired_at` 的真实先后，别信 cron 名义顺序。** 修:Agent2 cron → `0 20 * * *`(拉开 3h 缓冲吸收 ~1h 延迟)。教训延伸自"护栏架在机器上":**依赖执行顺序的自动化，缓冲要按观测到的最坏延迟留，不是按名义间隔留。** 另注:近几天 #74-#77 全是人工轨(owner 回 both→轮询器)产出，自主轨一直空转——owner 一忙不回信项目就停，正是这个竞态在背后。

- [2026-07-18] **做营销/宣传物料（小红书图文等）时，凡描绘产品的操作步骤、按钮名、交互动作，必须先在 index.html/app.js 里搜到确切文案与真实流程再落笔——别凭想象编交互。** 写小红书第2篇「拍照摘抄」时，我在演示图里画了「手指在屏幕上圈选划线句」的交互，owner 指出「我的 App 里没有实现」。真实的拍照摘抄流程（index.html:561-565、app.js:4763/2447）是：拍整页照片 → 两个按钮：`快速识别`＝OCR 整页全文、逐行可删（renderOcrLineSelector）／`AI 精识别`＝只识别你**在纸上用笔划线**的句子。所谓「划线」是读书时笔划的、不是屏幕圈选。教训同「断言现状前先核实」：**宣传图=对外承诺，编错交互=对用户撒谎**，比代码注释错更严重。已把这条铁律写进小红书自动生成脚本的 prompt（`~/.claude/scripts/paper-xhs-post.sh`），让无人值守的第3–7篇也强制先核代码再画。另：物料里复用的实拍图别跨帖重复（img-4 曾误用第1篇发过的实拍摘抄卡）——要用真实数据现做，且从库里挑摘抄时避开个人隐私向的（挑了《月亮虎》讲语言的书摘，避开《我走不出我的黑夜》那类沉重私人向）。
...
- [2026-07-17] 书籍去重第三条规则(OPT-118 真机第二次事故): 主标题 vs 主标题+副标题要视为同一本——书架 OCR 读封面全称《重走：在公路、河流和驿道上寻找西南联大》，库里只存《重走》。规则: **只有 ：/: 算副标题标记**(取冒号前的主标题)，`·` 绝不能算——它是系列/卷次分隔符(羊道·春牧场 ≠ 羊道·深山夏牧场)。且必须拿「一方的主标题」比「另一方的完整标题」，不能 main-vs-main：后者会把《明朝那些事儿：第一部/第二部》吞成一本(已用变异测试锁住)。三条规则合起来: 分隔符归一化 + 作者简写 + 副标题，前后端(app.js/app_server.py)镜像同步。
- [2026-07-17] 书籍去重的边界与设计原则(OPT-118 三轮真机测试后定案): 结构性差异可用规则精确判定(①分隔符 ②作者简写 ③副标题)，但**用字差异不行**——译名「本吉·沃特豪斯」vs「本吉·沃特斯豪斯」相似度 0.89、「伍尔夫/伍尔芙」0.67，而不同人的「余华/余桦」0.50、「李娟/李涓」0.50，**同一人与不同人的相似度区间重叠**，模糊匹配必然误伤(中文名短尤其危险)。定案: **不加模糊匹配**，改为「书名相同但作者对不上 → 标『可能重复：《X》/作者』、默认不勾、由人判断」。原则: 严格匹配用于自动决策，近似命中转为可见警告，静默误合(丢书)比重复(可见可修)严重得多。
- [2026-07-17] 豆瓣数据模型的两个坑(排查「豆瓣为什么导入重复书」时发现): ①**豆瓣按「版本」建 subject**，同一本书的不同出版社/译本是不同条目、作者名写法也不同(《钢铁是怎样炼成的》= subject/1013502「[苏] 尼·奥斯特洛夫斯基」+ subject/1894744「奥斯特洛夫斯基」)。爬虫按链接去重是对的(它无法知道两个 subject 是同一本)，**必须由 importDoubanCsv 的 isSameBook 兜住**——07-15 导入时还没有作者简写规则，所以漏了。②豆瓣作者字段写法特殊: 用**六角括号〔德〕**(不是[]或【】)、且**「著者」标签会混进作者名**。剥离标签时冒号必须是必需的分隔符(`(?:[:：]\s*|\s+)`)，写成可选会把「编者按」这种真名字啃成「按」(自己的测试当场抓到)。
- [2026-07-17] Do-Not-Repeat: 判断一本书是不是「空壳」，必须查**全部**用户字段——我只查了 rating/review/notes/doubanComment/摘抄/记录 就断言《荒原狼》是空壳，**漏了 startedAt/finishedAt/tags**，被 owner 当场纠正（那本有 Excel 录入的开始+读完日期和标签）。删除用户数据前，字段清单要完整：rating/startedAt/finishedAt/lastReadAt/tags/notes/review/doubanComment/coverImageUrl/totalPages/currentPage + 关联的摘抄/记录/关联。
- [2026-07-17] 重复书的正确处理是**字段级合并**而非删除——真实重复往往「各有各的数据」：Excel 导入的《荒原狼》有 startedAt/finishedAt(准)/tags/简介但评分卡在 notes 里(OPT-100 修复前的遗留)，豆瓣导入的同一本有结构化 rating 和短评但无开始日期、且 finishedAt 是**豆瓣标记日**(2025-11-01)而非真实读完日(2025-10-31)。=> 合并策略: 底本取信息量最高者(并列时作者 token 更多者优先，再并列取更早创建者)，非空字段互补，日期取更早者(豆瓣标记日常晚于实际读完)，标签取并集，摘抄/记录/关联改挂到底本，标题清掉多余书名号。脚本见本次会话(/tmp/merge_dupes.py 思路)。
- [2026-07-17] 书籍去重第五条规则——**版次后缀**(OPT-118 第五轮真机): 「第N版」是同一本书的**重印**,必须合并(库里《神经科学——探索脑（第4版）》vs 书架读到的《神经科学——探索脑》)。**但这条线极窄,只认「版」字**: 「第N部/卷/册」「上/下」「I/II」「结尾纯数字」都是**不同的书**,绝不能吃(花朵与探险 vs 花朵与探险2、第二性 I vs II、巴黎评论访谈 上/下)。副标题规则也故意只认冒号不认破折号,正是为了不误伤。已用变异测试锁死: 把正则从「版」放宽到「版|部|卷|册」立刻红 2 条。至此去重五条规则: 分隔符/作者简写/副标题/六角括号+著者标签/版次,前后端(app.js↔app_server.py)全部镜像。
- [2026-07-17] Do-Not-Repeat: **国籍标记别用白名单**。原实现维护 AUTHOR_NATIONALITY_LABELS 列表(中美英法德日俄意西葡…)，结果 owner 满架世界文学里 [阿根廷]/[哥伦比亚]/[苏]/[波]/[南非]/[以色列]/[捷克]/[智利]/[秘鲁] 全部漏网、国籍粘在名字上导致匹配失败。**方括号本身就是「这里是国籍」的信号**，不必再猜内容是不是已知国名 → 改为「开头的括号+≤6字内容」一律剥离(限长防止吃掉带括号的名字)。白名单只保留给「无括号」场景。
- [2026-07-17] 书籍去重第六条规则——**译名逐节缩写**: 中文出版界按「每节取首字」缩写译名(豪·路·博尔赫斯 = 豪尔赫·路易斯·博尔赫斯; 尼·奥斯特洛夫斯基 = 尼古拉·奥斯特洛夫斯基)。规则: **节数相同 + 每节互为前缀 + 至少2节**。⚠️「至少2节」是必须的——第一版漏了它，当场把「金」误合进「金庸」(自己的边界测试抓到)。单节中文名是完整的，不存在缩写约定。
- [2026-07-17] Do-Not-Repeat: 端点里**回包(_send_json)不能放进业务 try 块**。既有写法 `conn.close(); self._send_json(...)` 放在 try 内、except 里又 `append_log(conn,...)`——手机切走 App 时 iOS 挂起标签页断 socket，_send_json 抛 BrokenPipe → 落进 except → 拿**已关闭的 conn** 记日志 → ProgrammingError「Cannot operate on a closed database」**把真实错误盖掉**，还会把一次成功的 OCR 误记成失败。正确结构: try 只包业务调用，算出 (result,status) 后统一 append_log → conn.close() → _send_json。本次 4 处端点(books/ocr、books/shelf-ocr 等)都有此问题。
- [2026-07-17] Do-Not-Repeat: **apiFetch 的 catch 不能把所有网络失败都说成「后端没启动」**(原文案还叫用户去跑 ./scripts/dev_backend.sh——对手机用户纯误导)。手机上最常见的是「切走 App 导致请求中断」: iOS 挂起标签页+断 socket。⚠️**不能用 document.visibilityState 判断**——iOS 连 JS 都挂起，reject 处理器要等用户切回来才执行，那时已是 visible。正确做法: visibilitychange 里记 lastHiddenAt，catch 里比 `lastHiddenAt >= startedAt`(**必须 >=**，Date.now() 毫秒精度下两者可能同毫秒——测试当场抓到)。另: 在模块顶层注册 document 监听器要判 `typeof document.addEventListener === "function"`，否则 163 个既有测试的 document stub 全炸。
