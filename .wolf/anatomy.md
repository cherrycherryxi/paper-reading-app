...
> Auto-maintained by OpenWolf. Last scanned: 2026-08-31T17:01:41.639Z
...
## ../../../../../../../../var/folders/yb/15nyb3q91413vztqkb0jpz680000gn/T/tmp.KB7OI4wh/wt/.wolf/

- `memory.md` — Memory (~88970 tok)

## ../../../../../../../../var/folders/yb/15nyb3q91413vztqkb0jpz680000gn/T/tmp.KB7OI4wh/wt/optimization/

- `backlog.md` — Optimization Backlog (~41913 tok)
- `triage.md` — Triage (~1155 tok)

## ../../../../../../../../var/folders/yb/15nyb3q91413vztqkb0jpz680000gn/T/tmp.TliiFlhl/wt/.wolf/

- `memory.md` — Memory (~83528 tok)

## ../../../../../../../../var/folders/yb/15nyb3q91413vztqkb0jpz680000gn/T/tmp.TliiFlhl/wt/optimization/
...
- `memory.md` — Memory (~87627 tok)

## ../../../../../../../../var/folders/yb/15nyb3q91413vztqkb0jpz680000gn/T/tmp.tCpRInQE/wt/optimization/

- `backlog.md` — Optimization Backlog (~41856 tok)
- `triage.md` — Triage (~1100 tok)
...
- `big-breasts-wide-hips-review.md` — 读《丰乳肥臀》，我看到历史怎样落在人身上 (~601 tok)
- `build_big_breasts_wide_hips_docx.py` — set_run_font, add_page_number (~944 tok)
- `build_moon_tiger_docx.py` — set_run_font, add_page_number (~1522 tok)
- `build_my_night_docx.py` — set_font, add_page_number (~868 tok)
- `build_passion_spent_docx.py` — set_run_font, add_page_number (~903 tok)
- `build_younger_feet_docx.py` — set_run_font, add_page_number (~906 tok)
- `dsh-runtime-smoke-cordis.yml` (~182 tok)
- `moon-tiger-review.md` — 我忘了读过《月亮虎》，却因此记住了它 (~549 tok)
- `my-night-review.md` — 在记忆里，他们仍然年轻 (~369 tok)
- `passion-spent-review.md` — 被爱缠住的一生 (~408 tok)
- `younger-feet-review.md` — 在时间里重新理解失去 (~468 tok)
...
## ../../../../private/tmp/paper-opt152-155/

- `app_server.py` — import: static_asset_version, is_admin_username, build_sample_state, initialize_tool_schema_provider (~77280 tok)
- `chat.js` — STREAM_IDLE_TIMEOUT_MS: normalizePreferredBookValue, activeBookId, activeQuoteId + 20 more (~11593 tok)

## ../../../../private/tmp/paper-opt152-155/.wolf/

- `anatomy.md` — anatomy.md (~6065 tok)
- `memory.md` — Memory (~87766 tok)

## ../../../../private/tmp/paper-opt152-155/tests/agent/

- `prompt_builder_memories_test.py` — PromptBuilderMemoriesTests: setUp, test_opt148_injects_global_and_current_book_confirmed_memories, t (~687 tok)

## ../../../../private/tmp/paper-opt152-155/tests/frontend/

- `chat-agent-approval.test.js` — test: createElementStub, innerHTML, innerHTML + 3 more (~6061 tok)

## ../../../../private/tmp/paper-pr117-status-text/

- `chat.js` — STREAM_IDLE_TIMEOUT_MS: normalizePreferredBookValue, activeBookId, activeQuoteId + 20 more (~11618 tok)

## ../../../../private/tmp/paper-pr117-status-text/tests/frontend/
...
- `app.js` — Declares AUTH_TOKEN_KEY (~70701 tok)
- `chat.js` — STREAM_IDLE_TIMEOUT_MS: normalizePreferredBookValue, activeBookId, activeQuoteId + 20 more (~11582 tok)
...
## ../../../../tmp/paper-pr136-review.2gjcP3/optimization/

- `backlog.md` — Optimization Backlog (~46041 tok)
- `triage.md` — Triage (~2255 tok)
...
## ../../../../tmp/paper-reconcile-20260824.slZXGC/optimization/

- `backlog.md` — Optimization Backlog (~44746 tok)
- `triage.md` — Triage (~1789 tok)
...
## .agents/skills/ux-reviewer/

- `SKILL.md` — UX/UI Review (~395 tok)
...
- `codex-address-review.yml` — CI: Ask Codex to address review feedback (~1180 tok)
...
- `backlog.md` — Optimization Backlog (~46813 tok)
- `explore.md` — Exploration — new optimization directions (~88380 tok)
...
- `dev_backend.py` — backend_python, check_backend_environment, check_mcp_server, iter_watched_files + 6 more (~1692 tok)
- `dev_backend.sh` (~164 tok)
- `generate_thumbnails.py` — 为 uploads 下的图片生成书单用缩略图（<stem>.thumb.jpg，最长边 400px）。 (~1969 tok)
- `run_backend_service.sh` — Unattended backend runner for launchd (stable serving, NOT the dev watcher). (~132 tok)
- `start_backend.sh` (~372 tok)
- `start_mcp.sh` (~154 tok)

## scripts/codex/

- `cloud-setup.sh` — Codex Cloud environment setup for paper-reading-app. (~85 tok)
- `deploy-prod.sh` — 生产发布入口：生成版本说明、推送 feature/agent 和 main、更新并重启 Prod。 (~888 tok)
...
- `nightly-explore.sh` — 每日 05:00，07:00 补偿重试：执行夜间 Agent3（Explore）。 (~1181 tok)
...
- `release-hook.sh` — 生产发布 hook：根据本次 Prod 版本与上一版本的提交记录生成更新说明。 (~922 tok)
- `weekly-prod-release.sh` — 每周日 17:00：仅经统一发布脚本将 feature/agent 发布到 Prod。 (~1380 tok)
...
- `com.huangnanqi.paper-codex-nightly-explore.plist` (~296 tok)
...
- `__init__.py` (~1 tok)
- `account_export_delete_test.py` — Regression tests for account data export + deletion (P0 GDPR/PIPL compliance). (~2404 tok)
- `action_executor_atomic_test.py` — Regression test for OPT-029 (Layer A): ActionExecutor.execute_action() must (~1554 tok)
...
- `agent_backend_property_test.py` — Tests: property_database_migration_creates_expected_schema, property_request_validation_ordering, property_parse_failure_visibility, parser_salvage... (~15543 tok)
- `agent_backend_reliability_test.py` — Tests: chat_response_includes_trace_and_persists_pending_action, chat_parse_failure_returns_degraded_status_without_actions, chat_validation_failur... (~7706 tok)
...
- `agent_golden_set_eval.py` — GoldenSetEvaluator: close, request_json, run_case, fake_deepseek + 8 more (~3540 tok)
- `agent_link_thought_test.py` — Tests: chat_valid_link_thought_returns_pending_action, chat_missing_thought_field_rejected, chat_missing_source_id_rejected, chat_wrong_type_source... (~3644 tok)
...
- `codex_weekly_automation_test.py` — Contract tests for the Codex Sunday/Monday local automations. (~2890 tok)
...
- `db_concurrency_test.py` — Regression tests for SQLite tuning (P3 commercialization). (~560 tok)
...
- `request_body_size_cap_test.py` — View: get (~1891 tok)
- `sample_state_test.py` — 示例内容（新用户 onboarding）后端逻辑测试。 (~481 tok)
...
- `tool_schema_provider_test.py` — Tests: for_prompt_includes_all_action_types_and_hides_infra_params, action_types_preserve_schema_order, validate_action_data_accepts_valid_payloads... (~1745 tok)

## tests/frontend/

- `a11y-baseline.test.js` — Regression tests for the a11y baseline pass: OPT-018 (prefers-reduced-motion), (~770 tok)
