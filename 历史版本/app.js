...
function renderStats() {
  const stats = [
    {
      label: "阅读中",
      value: state.books.filter((book) => book.status === "reading").length,
    },
    {
      label: "已读完",
      value: state.books.filter((book) => book.status === "finished").length,
    },
    {
      label: "摘抄卡片",
    ...
    //       ? Math.round(
    //           state.sessions.reduce((sum, session) => sum + session.minutes, 0) / state.sessions.length
    //         )
    //       : 0,
    // },
  ];
...
(function () {
  ...
  function activateTab(tabName) {
    ...
    // 探讨 tab 直接打开对话窗
    if (tabName === 'chat') {
      document.getElementById('chatOverlay')?.classList.remove('is-hidden');
      // 切回上一个 tab，因为 chat 有自己的弹窗
      setTimeout(() => activateTab('books'), 50);
    }
  }
