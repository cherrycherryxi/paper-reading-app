...
| OPT-002 | 「书单」加书支持拍照 OCR 识别 | P1 | M | done (PR #17) | No `/api/books/ocr` endpoint yet; `call_kimi_vision`/`call_deepseek` reusable. Need new endpoint returning `{title, author, tags[]}` + camera input in `bookDialog`. Touch: app_server.py, index.html, app.js. |
...
| OPT-015 | 摘抄卡面内容难以分辨 UI 优化 | P2 | M | done (PR #16) | Requires `openwolf designqc` screenshot + targeted CSS fixes. Touch: styles.css (quote card selectors). |
