...
**OPT-069 — call_deepseek_stream() 无重试：主聊天路径遇瞬断即报错（P1/S）**

理由：（1）**预算余 1 槽** — 近 7 天（2026-06-18→2026-06-25）共 3 个 `auto/` PR（#49 opt-062 2026-06-24、#48 opt-052 2026-06-19、#47 opt-054 2026-06-18），上限 4，余 1。（2）**OPT-062/063 已在 PR #49 覆盖**，下一候选按优先级落到 OPT-069。（3）**northstar「强」**——`call_deepseek()`（OPT-012，已修）有 3 次指数退避，streaming 主聊天路径遇 429/502 直接抛 RuntimeError、用户见错误弹窗，两路径行为不一致；此次预算最后 1 槽应优先填最高 northstar 项。（4）S 复杂度——仅将 `urlopen()` 建连步骤包进 `for attempt in range(DEEPSEEK_MAX_ATTEMPTS):` 循环，镜像 `call_deepseek()` 已验证模式（line 3192-3220），再为 streaming 路径在 `tests/agent/deepseek_retry_test.py` 新增 `DeepseekStreamRetryTest`。

关键文件：`app_server.py:3226-3270`（`call_deepseek_stream` — 将 `urlopen` 调用移入重试循环，HTTPError/TimeoutError/URLError 处理镜像 `call_deepseek`）；`tests/agent/deepseek_retry_test.py`（新增 streaming 路径 429/503 重试 + 耗尽断言）。

northstar/signal：northstar「强」（OPT-069 原评级），无 signals.md 直接条目，但与 OPT-012 同根同修——修复后两路径行为对等，消除「偶发 429 使整个聊天失败」的用户信任损失。

## Prioritized backlog

| id | title | priority | complexity | status | notes |
|----|-------|----------|------------|--------|-------|
| OPT-062 | 确认对话框 Escape 关闭后监听器残留，可触发错误删除 | P1 | S | in-progress (PR #49) | northstar「中」，Theme 1「零丢失」验收直接威胁；data safety 类。`showConfirmDialog`（`app.js:2286-2297`）缺 `cancel` 事件清理，6+ 删除路径受影响；`deleteBook` 弹窗（`app.js:2120`）另有独立缺口；各追加 1 行 `addEventListener("cancel", cleanup, { once: true })`。同 PR 含 OPT-063。 |
| OPT-063 | compress_chat_history API 失败时写入截断历史，永久丢失旧聊天记录 | P1 | S | in-progress (PR #49) | northstar「中」，Theme 2「回顾有价值」前提数据；data safety 类（与 OPT-040/041 同类）。`app_server.py:2279-2292`：`save_state` 移入 `try` 块（压缩成功才持久化），`except` 改直接 `return history`；约 4 行重排，零副作用。与 OPT-062 同 PR。 |
| OPT-069 | call_deepseek_stream() 无重试：主聊天路径遇瞬断即报错 | P1 | S | in-progress (PR #50) | northstar「强」——主聊天（Theme 2 核心入口）遇 429/502 报错是对用户信任的直接伤害；`call_deepseek()` 已有 3 次重试，streaming 路径零重试，两路径不一致。`app_server.py:3226-3270`：将 `urlopen()` 调用放入 `for attempt in range(DEEPSEEK_MAX_ATTEMPTS):` 循环，镜像 `call_deepseek()` 模式（lines 3192-3220）；`tests/agent/deepseek_retry_test.py` 新增 `DeepseekStreamRetryTest` 类。S 复杂度，零逻辑变更。 |
...
| OPT-059 | Session 日期预填 UTC 日期，UTC+8 凌晨记录日期差一天 | P1 | S | triaged | northstar「中」，Theme 1 数据准确性；影响 roadmap §2「本周使用天数」指标统计。`app.js:2261`：`toISOString().split("T")[0]` → `new Intl.DateTimeFormat("sv").format(new Date())`（sv locale 返回本地时区 YYYY-MM-DD，无 polyfill）。UTC+8 凌晨高频场景。 |
...
| OPT-066 | 编辑 Session 未同步书籍进度字段（currentPage/lastReadAt/updatedAt） | P2 | S | triaged | northstar「中」，Theme 1 数据准确性；编辑 session 的 if(existingId) 分支（`app.js:2029-2037`）只更新 session 本体，未重算 `book.currentPage/lastReadAt/updatedAt`，也不触发 finished 状态判断；对比新建分支（`app.js:2046-2055`）完整同步。约 5 行补全，S 复杂度。 |
