...
|----|-------|----------|------------|--------|-------|
| OPT-008 | summarize_metrics json.loads 无 try-except | P0 | S | in-progress (PR #11) | Single unguarded `json.loads(row["dimensions"])` at line 2438. One corrupt row → permanent 500 on all monitoring endpoints for that user. Fix: wrap in try/except json.JSONDecodeError + continue. Touch: app_server.py:2435-2438. |
