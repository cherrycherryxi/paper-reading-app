...
Last triaged: 2026-06-28

## Next up

本周实现预算已满（近 7 天已有 4 个 auto PR，上限 4），本次不指派

（近 7 天 auto/ PR：#49 opt-062 2026-06-24、#50 opt-069 2026-06-25、#51 opt-068 2026-06-26、#53 opt-074 2026-06-27。预算复位后首推候选：**OPT-064**——PromptBuilder 发送 ocrText 字段每次浪费数百至数万 token，P1/S，直接降低 API 成本，northstar「中」，与 OPT-020/047 同类但影响规模更大。）
...
| OPT-053 | Session 统计条仅在搜索时显示——日常浏览看不到累计阅读数据 | P2 | S | triaged | `app.js:1415-1425`：无搜索时全量计算并常驻显示，有搜索时展示过滤子集（现有逻辑）。northstar「中」，roadmap §2 可观测代理指标；signal 2026-06-26（记阅读活跃）佐证可观测性价值。3 行改动，无 HTML/CSS/后端变动。 |
...
| OPT-057 | 「动态」Tab 时间线硬限 10 条，积累后无法看到更多历史 | P2 | S | triaged | `app.js:1413` 底部加「查看更多」按钮，`slice(0, count+10)`，局部变量跟踪当前展示数。northstar「中」，Theme 2；当前 Theme 1 期 P2。 |
...
| OPT-076 | renderTimeline() 硬上限 10 条且无告知，阅读历史超 10 次后早期记录不可见 | P2 | M | triaged | northstar「中」，Theme 2「回顾有价值」；`app.js:1413`：`allSorted.slice(0, 10)` 硬截断，无分页/load-more；方案 B：`displayLimit` 模块变量 + 「加载更多（共 N 条）」按钮；Touch: `app.js:1397-1425`（renderTimeline 截断逻辑 + DOM 渲染块）。Theme 2 验收期前的前置缺陷，与 OPT-057 同类，可合并一 PR。 |
