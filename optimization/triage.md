...
**状态：in-progress · PR #131 待审查与合入核验**
...
- OPT-167 为 P1/S：当前 `proposals:[null]` 会在字典展开处抛错并令整个 run 失败；修复是结构校验与降级容错，验收边界明确，符合夜间局部正确性任务，故作为本次唯一 Next up。
...
|----|-------|----------|------------|--------|-------|
| OPT-167 | 深度共读结果内部结构未校验，畸形建议会令整次任务失败 | **P1** | S | **in-progress** | PR #131 待审查与合入核验；过滤坏成员、保留合法建议与其余研究结果 |
