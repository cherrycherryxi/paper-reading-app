...
Last triaged: 2026-07-07

## Next up

**W28 Item 1 — 录入修复包 (OPT-058 + OPT-061 + OPT-066 + OPT-090 + OPT-084 + OPT-091)**

in-progress (PR #59, auto/opt-058-061-066-090-084-091-entry-bundle, 2026-07-07)

实现预算检查：近 7 天 `auto/` PR 共 **2 个**（#54 OPT-059，2026-07-03；#55 OPT-064，2026-07-05），距上限 4 还余 **2 个槽位**，可以指派。（注：auto/opt-079-080-connection-ux 和 auto/opt-078-custom-tags-sync 分支已推送，PR 尚未在 GitHub 创建，故不计入本次 7 天窗口。）

选择理由：
1. **roadmap W28 item 1 显式命名为「唯一焦点第一件事」**：roadmap §2 原文「先把它们打包成一个 PR 一次清掉，不再逐周滚动」——W28 item 1 明确列出 OPT-058+061+066+090+084+091，全为 S 级、同路径、同文件，合并成一个 PR 是 roadmap 的明确指令。
2. **OPT-079/080（PR #56）和 OPT-078（PR #57）已被上一轮 Agent2 认领**：分支已推送远端，不与本 bundle 文件路径重叠（OPT-079/080 改摘抄菜单+关联对话框；OPT-078 改 state/sanitize_state；录入修复包全在 session/quote dialog + Timeline 排序路径）。
3. **Theme 1「采集顺滑」收尾**：W27 遗留 4 项全部在此 bundle 内（OPT-058 摘抄对话框 focus、OPT-061 session 对话框 focus、OPT-066 editSession 进度回写、OPT-090 editSession 日期时区），加 OPT-084（startPage 预填）和 OPT-091（Timeline 排序）同路径搭车，一次清零所有录入摩擦积压。
4. **signal 2026-06-26 佐证**：编辑 session 不同步进度（OPT-066）、date 字段 UTC 偏移（OPT-059 已修、OPT-090 是编辑路径对称 bug）都有真实使用信号，W27 唯一焦点就是这条路径。

关键文件：
- `app.js:2248, 2283`（openNewQuoteForBook、editQuote — `showModal()` 后加 `requestAnimationFrame(() => document.getElementById("quoteContent")?.focus())`）
- `app.js:2142, 2262`（editSession、openNewSessionForBook — `showModal()` 后加 `requestAnimationFrame(() => document.querySelector('#sessionDialog [name="startPage"]')?.focus())`）
- `app.js:2029-2037`（editSession 编辑分支 — 回写 `book.currentPage`/`book.lastReadAt`/`book.updatedAt`/`finished`，与新建分支 `app.js:2046-2055` 对称）
- `app.js:2412`（editSession 日期预填 — `toISOString().split("T")[0]` → `isoToDateInput(session.date)`，复用已有 helper）
- `app.js:2436`（openNewSessionForBook — `startPage value=""` → `book.currentPage > 0 ? book.currentPage + 1 : ""`）
- `app.js:1439`（renderTimeline 排序 — `localeCompare` → `(Date.parse(b.date)||0) - (Date.parse(a.date)||0)`）

roadmap 依据：roadmap.md §2 W28「本周具体事项第 1 条：录入修复包（OPT-058+061+066+090+084+091）」；signals.md 2026-06-26 session 录入路径摩擦佐证。

## Prioritized backlog

| id | title | priority | complexity | status | notes |
|----|-------|----------|------------|--------|-------|
| OPT-058 | 摘抄对话框 showModal() 后未 focus() 文本区，移动端每次多点击一次 | P1 | S | in-progress (PR #59, 2026-07-07) | roadmap W28 录入修复包 item；Theme 1「采集顺滑」核心录入路径。`app.js:2248`（openNewQuoteForBook）和 `app.js:2283`（editQuote）各加 `requestAnimationFrame(() => document.getElementById("quoteContent")?.focus())`。白天 bundle 项。 |
| OPT-061 | Session 对话框 showModal() 后无 focus()，移动端须额外点击才能开始输入 | P1 | S | in-progress (PR #59, 2026-07-07) | roadmap W28 录入修复包 item；signal 2026-06-26 佐证；Theme 1「采集顺滑」每日触点。`app.js:2142`（editSession）和 `app.js:2262`（openNewSessionForBook）末尾各追加 `requestAnimationFrame(() => document.querySelector('#sessionDialog [name="startPage"]')?.focus())`。白天 bundle 项。 |
| OPT-066 | 编辑 Session 未同步书籍进度字段（currentPage/lastReadAt/updatedAt） | P1 | S | in-progress (PR #59, 2026-07-07) | roadmap W28 录入修复包 item；signal 2026-06-26 佐证；`app.js:2029-2037` 的 `if(existingId)` 分支只更新 session 本体，未重算书籍进度；对比新建分支（`app.js:2046-2055`）完整同步。约 5 行补全。白天 bundle 项。 |
| OPT-090 | editSession() 日期预填用 toISOString() 而非 isoToDateInput()，编辑路径存在与 OPT-059 对称的时区 bug | P1 | S | in-progress (PR #59, 2026-07-07) | NEW（explore E145，2026-07-03）；roadmap W28 录入修复包 item；OPT-059 的编辑路径对称 bug；`editSession()` 中日期预填直接用 `value.split("T")[0]`，UTC+8 凌晨跨天时显示错误；改用已有的 `isoToDateInput()` helper（`app.js:477-484`）。白天 bundle 项。Touch: `app.js:2412`。 |
| OPT-091 | renderTimeline() 用 localeCompare 排序 session，OPT-037 的书单修复未覆盖 Timeline | P1 | S | in-progress (PR #59, 2026-07-07) | NEW（explore E146，2026-07-03）；OPT-037 同类遗漏（书单修复未同步 Timeline）；`renderTimeline()` 中 session 排序仍用字符串 localeCompare，agent/OCR 创建的 session 在东八区比手动创建的 UTC 字面大 ~8h，恒排前；改 `Date.parse()` 数值比较（降序 b-a）。Touch: `app.js:1439`。白天 bundle 项。 |
| OPT-092 | matchBooks() 忽略 book.tags / book.notes，书单按主题/标签搜索零结果 | P1 | S | triaged | NEW（explore E150，2026-07-04）；**signal 2026-07-03** 直接佐证（为读书会找书，书单搜「成长」零结果）；roadmap W28 Theme 2「检索修通」第一 PR 显式命名；`app.js:1160-1163` 追加 `(book.tags||[]).some(t => fuzzyMatch(t,query)) \|\| fuzzyMatch(book.notes||"",query)`；2–3 行，纯前端，零后端变更。白天 bundle 项（与 OPT-083/056/088/096/097 合并一 PR）。 |
...
| OPT-079 | 摘抄卡 ⋯ 菜单增加「建立关联」直达入口；来源自动预填当前摘抄 | P1 | S | in-progress (PR #56, auto/opt-079-080-connection-ux, 2026-07-07) | **signal 2026-06-29** 佐证「来源没自动填入当前摘抄（还得手动选）」；Theme 2；`app.js:1568-1571` 菜单加 `connect` 选项 + `app.js:4967-4972` handler 新增分支调用 `openConnectionDialog({sourceType:"quote",sourceId:id})`；与 OPT-080 合并一 PR。 |
| OPT-080 | 关联对话框目标摘抄标签截断至 32 字 + CSS 双重省略导致同书摘抄无法辨识 | P1 | S | in-progress (PR #56, auto/opt-079-080-connection-ux, 2026-07-07) | **signal 2026-06-29** 直接佐证「目标显示不完整、看不清内容、找不到想关联的那一条」；Theme 2「建立关联」核心路径；`app.js:4408` `slice(0,32)`→`slice(0,60)` + `app.js:4442` 去掉 `white-space:nowrap`，S 级修复；与 OPT-079 合并一 PR。 |
| OPT-078 | 自定义摘抄标签仅存 localStorage，跨设备/跨网址不同步，导出包中不存在 | P1 | M | in-progress (PR #57, auto/opt-078-custom-tags-sync, 2026-07-07) | **signal 2026-06-29** owner 换网址后标签丢失直接痛点；northstar「中」；Theme 2「按主题检索」基础。`app.js:480-484`（getCustomQuoteTags/saveCustomQuoteTags）改双写 state；`app_server.py:633-667`（sanitize_state）加 `customQuoteTags` 字段。M 复杂度（前后端双改）。 |
...
| OPT-084 | openNewSessionForBook() 从不预填 startPage，每次录入需手动输入已知起始页 | P2 | S | in-progress (PR #59, 2026-07-07) | explore E137 2026-07-01；中北极星 Theme 1；session 录入是 W28 焦点路径，startPage 预填 = book.currentPage+1 减少每次录入 1–2 次交互；`app.js:2436` value="" → `(book.currentPage>0?book.currentPage+1:"")`；建议与同路径 bundle 搭车。 |
...
| OPT-094 | addSession() pagesRead 计算差一，统计数据永远少计一页 | P2 | S | triaged | NEW（explore E148，2026-07-05）；northstar 弱（数据准确性）；三处均漏 `+1`：`app.js:2311`（addSession）、`app.js:2318`（editSession）、`app.js:1456`（统计栏汇总）；纯前端 3 行，无 DB/后端变更；建议与 OPT-084/093 搭车同一 PR。 |
| OPT-095 | 新建摘抄对话框页码字段从不预填 book.currentPage | P2 | S | triaged | NEW（explore E155，2026-07-05）；northstar 弱-中，Theme 1 小摩擦消除；`app.js:2520`（openNewQuoteForBook）将 `value=""` 改为读取 `state.books.find(b=>b.id===bookId)?.currentPage`；2–3 行，纯前端；建议与 OPT-084（session startPage 预填）搭车同一 PR。 |
...
| OPT-096 | renderConnections() 搜索 haystack 缺少 c.tags，关联标签无法被搜索命中 | P2 | S | triaged | NEW（explore E135/E161，2026-07-06）；northstar「弱-中」，Theme 2「回顾有价值」；`app.js:862-866` haystack 追加 `...(c.tags||[])`；1–2 行，纯前端；建议与 OPT-092（matchBooks tags）、OPT-097（matchBooks review）合并「搜索字段补全 bundle」PR。 |
| OPT-097 | matchBooks() 不搜索 book.review，OPT-087 新增字段对搜索路径完全不可见 | P2 | S | triaged | NEW（explore E158，2026-07-06）；northstar「弱-中」，Theme 2；OPT-087 同日上线 review 字段但搜索未同步；`app.js:1165` 末尾追加 `\|\| fuzzyMatch(book.review||"",query)`；1 行，零后端影响；建议与 OPT-092、OPT-096 合并「搜索字段补全 bundle」PR。 |
...
| OPT-087 | 摘抄/书/思想碰撞「一键生成分享图」(内容卡自传播增长引擎) | P2 | L | in-progress (2026-07-06 三版式全落地) | northstar「强」；owner 2026-07-02 为读书会主动提出。2026-07-06 **方案 A 纯 Canvas** 三版式全部落地（摘抄卡/思想碰撞卡/书卡，各自 ⋯菜单/详情/卡片入口→shareCardDialog 预览/下载），三版均 headless Chrome 真渲染验收；测试 `share-card.test.js` 9 例。**剩余**:真机 QC + 分享埋点。详见 backlog progress。 |
...
| OPT-085 | 书封面上传未压缩（单张可达 4.6MB），拖慢移动端书单加载 | P1 | M | done (2026-07-06) | **重定范围**：前端压缩早于 signal 已实现（2026-05-14，三路径 1200px/q0.85），描述过时；实际做的是历史存量清理 `generate_thumbnails.py --recompress-originals`，dev 12 张 36MB→4.6MB；测试 `recompress_originals_test.py` 6 例；**prod 待各跑一次**。详见 backlog done note。 |
