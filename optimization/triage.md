...
|----|-------|----------|------------|--------|-------|
| OPT-020 | PromptBuilder 注入无关 existing_connections | P1 | S | triaged | `app_server.py:2258` 仍无条件注入 connections[:20]（代码已确认）。1 行条件判断，零行为变更，直接省 token。Touch: `app_server.py:2258`。 |
