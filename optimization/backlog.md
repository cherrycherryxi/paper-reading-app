...
- status: done (2026-07-13, PR #62 squash-merged into feature/agent；方案 B：模块级 sessionDisplayLimit=10 + 「加载更多（还有 N 条，共 M 条）」按钮，同时修复截断与「无告知」；一并关闭重复项 OPT-057)
- area: frontend
...
- status: done — PR #69（与 OPT-114 合并一 PR）已合入 feature/agent [2026-07-16]
- area: backend
...
- status: done — PR #69（与 OPT-113 合并一 PR）已合入 feature/agent [2026-07-16]
- area: frontend
...
- priority: P1
- size: M
- **可行性验证结论（2026-07-17，owner 真实书架 3 张照片实测 kimi-k2.5）**：共识别 **57 本，书名准确率约 90%+**，远超预设的 60% 决策门槛 → **方案可行**。耗时 20–28s/张（需进度态 UI）。图片需 thumbnail 到 **2000px**（现有 `resizeImageToDataUrl` 默认 1200px 不足以识别书脊小字）。**实测发现三个必须在实现中处理的问题**：①**副标题被拆成独立一本**（《重走：在公路、河流和驿道上寻找西南联大》拆出「在公路、河流和驿道上寻找」；《厌女：日本的女性嫌恶》拆出「日本的女性嫌恶」）→ 提示词必须明确「副标题属同一本，不得拆分」；②**平放/被遮挡的书会幻觉**（顶部横放的《我弥留之际》被识别成《当我们谈论爱情时我们在谈论什么》，且 confidence 给到 0.70）→ **confidence 不可靠，不能仅靠它过滤**；③少量遗漏（被遮挡的《女友杜拉斯》未输出）。=> **勾选确认 UI 是必需项而非可选**，且「默认全选」需谨慎（建议高置信默认选中、中低置信默认不选但可勾）。
...
- priority: P2
- size: S
- northstar: 中——Theme 2「回顾有价值」AI 查询层。`review` 字段是 2026-07-06 新增功能，OPT-105 豆瓣导入后 `doubanComment` 已补入 `all_books_summary`，`review` 应同步对齐；1 行修复，直接提升跨书回顾类 AI 答复质量。
- description: `app_server.py:2607-2612` 的 `all_books_summary` dict 包含 `id, title, author, status, rating, finishedAt, doubanComment`，但 `review`（用户手写或 AI 生成读后感）缺席。单本书上下文（line 2592 直传完整 book 对象）无此问题；缺口仅在跨书摘要路径。用户询问「帮我回顾最近读了什么」或「推荐一本」时，AI 看不到用户自己的读后感，只能泛泛而谈，而更低价值的 `doubanComment` 反而可见。
- why: OPT-118 补入 `doubanComment` 时应同步补 `review`，属遗漏。修复后跨书回顾类对话可直接引用用户自己的评价，明显提升 Theme 2 回顾质量。
...
- priority: P2
- size: S
- northstar: 中——Theme 1「采集顺滑」补录路径正确性。`startedAt` 自动推算是 OPT-074 核心功能之一，但「追溯补录更早日期」的语义不正确：守卫只保护「空值写入」，把「写入更早日期以修正开始时间」的合法场景也阻断了。OPT-105 豆瓣导入后 owner 可能有大量历史书目需要逐本补录更早的阅读记录，此守卫会导致 `startedAt` 始终卡在「第一次补录时的日期」而非「实际最早日期」。
...
- status: done (PR #122, merged 2026-08-14 — `3bdf431`；后续主页迭代 `35f9e99` 将长期记忆表单、Excel 导入与拍书架入口直接落在“我的”主页；`tests/frontend/me-homepage-entry.test.js` 覆盖入口、保存/删除刷新与移动端字号)
...
- description: 已完成。当前 `index.html:251-287` 的“阅读工具”区直接承载长期记忆表单、Excel 导入与拍书架识别；`app.js:423-445` 实现记忆渲染和保存，`app.js:6390-6405` 绑定主页入口交互。
...
- status: done (PR #126, merged 2026-08-18 — `ad85cd5`；启动时原子收口遗留 `CREATED/RUNNING`，记录重启中断原因与 `RUN_FAILED` 事件，幂等及终态保护测试已落地)
...
- status: done（✅ PR #138 / `6874095` 已合入 [2026-09-01]；`showConfirmDialog` 加可选 `inputConfig` 文本输入槽，`deleteAccount` 弃用 `window.prompt`，前端回归断言不再用 prompt 且输入值透传 `onConfirm`）
