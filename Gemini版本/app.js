...
detectOcrCapability();
const newBookId = Date.now(); // 假设这是你生成的ID
render();
// 【新增：跳转与滚动】
if (window.innerWidth <= 768) {
  activateTab('books'); // 切换到书单
  setTimeout(() => {
    // 这里的选择器要对应你 renderBooks 生成的书籍卡片 ID
    const card = document.querySelector(`[data-book-id="${newBookId}"]`);
    if (card) {
      card.scrollIntoView({ behavior: 'smooth', block: 'center' });
      card.classList.add('highlight-flash'); // 加个高亮效果
    }
  }, 100);
}
