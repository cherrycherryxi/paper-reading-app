...
class AgentBackendReliabilityTests(unittest.TestCase):
    ...
    def fetch_action(self, action_id):
        conn = app_server.get_conn()
        action = app_server.ActionStateMachine().get_action(conn, action_id, self.user_id)
        conn.close()
        return action
    ...
    def test_approval_endpoint_returns_failed_status_when_execution_raises(self):
        ...
        app_server.TraceManager().create_trace(
            conn,
            trace_id=trace_id,
        ...
        action = app_server.ActionStateMachine().create_action(
            conn,
            trace_id,
            self.user_id,
        ...
        original_execute = app_server.ActionExecutor.execute_action

        def fake_execute(self, conn, user_id, action):
            return app_server.ExecutionResult(False, app_server.ACTION_STATUS_FAILED, action, error_message="boom")

        app_server.ActionExecutor.execute_action = fake_execute
        try:
            status, payload = self.request_json(
                "POST",
            ...
            app_server.ActionExecutor.execute_action = original_execute
    ...
    def test_metrics_collection_tracks_requests_actions_and_failures(self):
        ...
        app_server.TraceManager().create_trace(conn, trace_id=trace_id, user_id=self.user_id, message="manual", book_id="book-1")
        action = app_server.ActionStateMachine().create_action(
            conn,
            trace_id,
            self.user_id,
        ...
        conn.close()
        original_execute = app_server.ActionExecutor.execute_action
        app_server.ActionExecutor.execute_action = lambda self, conn, user_id, action: app_server.ExecutionResult(False, app_server.ACTION_STATUS_FAILED, action, error_message="exec fail")
        try:
