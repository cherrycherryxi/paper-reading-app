...
test("读后感字段：新增书与编辑书两个表单都能录入，且各自保存到 book.review", () => {
  ...
  // （saveBookEdit 的落库行为由 ai-review-source-badge.test.js 真跑断言；OPT-101 后
  //   它先算来源标记再赋值，故这里只断言 review 取自表单的 nextReview。）
  assert.ok(/review: String\(formData\.get\("review"\)/.test(appSource), "addBook 应保存 review");
  assert.ok(/const nextReview = String\(formData\.get\("review"\)/.test(appSource)
    && /book\.review = nextReview/.test(appSource), "saveBookEdit 应保存 review");
});
...
// OPT-108 修正：提示词「目标篇幅」与分享图「硬截断上限」必须【拆开】，上限要 > 目标，
// 给模型的超写留余量。曾合成同一个 200，模型写到 207 就被切尾（问题根源）。
test("AI 读后感提示词目标 < 书卡截断上限（留超写余量，避免切尾）", async () => {
  const { hooks, ctx } = buildContext();
  const target = hooks.BOOK_REVIEW_TARGET_CHARS;
  const max = hooks.BOOK_REVIEW_MAX_CHARS;
  assert.equal(typeof target, "number");
  assert.equal(typeof max, "number");
  assert.ok(max > target, "分享图截断上限必须大于提示词目标篇幅（容忍模型超写）");

  // 提示词引用「目标」常量，而不是「上限」常量、也不是字面量
  assert.ok(
    /写一段简短的读后感（100-\$\{BOOK_REVIEW_TARGET_CHARS\}字）/.test(appSource),
    "generateBookReview 的提示词应引用 BOOK_REVIEW_TARGET_CHARS"
  );

  // 模型写到「目标 + 少量超写」（仍 ≤ 上限）时，读后感应完整画出、不被截断
  const overshoot = "感".repeat(target + Math.min(7, max - target));
  await hooks.renderBookShareCard({ id: "b1", title: "活着", author: "余华", status: "finished",
    review: overshoot, notes: "", tags: [] });
  const drawn = ctx.__draws.texts.join("");
  assert.ok(drawn.includes(overshoot), "目标篇幅+少量超写的读后感应完整出现在书卡上");
  assert.ok(!drawn.includes(`${overshoot}…`), "未超上限不应加省略号");
});
