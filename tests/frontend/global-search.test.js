...
test("globalSearch renders only matching books, ignoring active filters and quote matches", () => {
  const hooks = createHarness();
  ...
  assert.deepEqual(hooks.getSectionHeaders(), [""]);
  assert.equal(hooks.getBooksResultCount(), "找到 1 本书籍");
...
test("property: result count text matches rendered book totals only", () => {
  const hooks = createHarness();
  hooks.setCurrentUser({ id: "user-1" });
  const books = [
  ...
  const matched = hooks.getBooksResultCount().match(/找到 (\d+) 本书籍/);
  assert.equal(Number(matched[1]), books.length);
});

test("property: search results render direct book cards without quote sections", () => {
...
test("renderSearchResults ignores quote matches when there are no matching books", () => {
  const hooks = createHarness();
  hooks.setCurrentUser({ id: "user-1" });
  ...
  assert.equal(hooks.getBooksResultCount(), "找到 0 本书籍");
  assert.match(hooks.getAllBooksListMarkup(), /没有找到匹配的书籍/);
  assert.doesNotMatch(hooks.getAllBooksListMarkup(), /黑暗森林|摘抄/);
});

test("globalSearch shows book-only empty state when no books match", () => {
  const hooks = createHarness();
  hooks.setCurrentUser({ id: "user-1" });
  hooks.setState({
  ...
  assert.equal(hooks.getBooksResultCount(), "找到 0 本书籍");
  assert.match(hooks.getBooksListMarkup(), /没有找到匹配的书籍，试试其他关键词/);
});
